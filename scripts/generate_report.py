#!/usr/bin/env python3
"""
Generate counseling & psychotherapy daily report HTML using Zhipu AI.
...
SYSTEM_PROMPT = (
    "你是臨床心理學與心理治療領域的資深研究員與科學傳播者。你的任務是：\n"
...
def analyze_papers(api_key: str, papers_data: dict) -> dict:
    ...
    prompt = f"""以下是 {date_str} 從 PubMed 抓取的最新臨床心理/心理治療/創傷文獻（共 {paper_count} 篇）。
...
每篇 paper 的 tags 請從以下選擇：CBT、DBT、EMDR、創傷治療、PTSD、解離症、憂鬱症、焦慮症、強迫症、成癮、伴侶/婚姻治療、家族治療、親職教養、兒童青少年、自閉症、ADHD、飲食障礙、自殺防治、正念、藝術治療、遊戲治療、沙盤治療、敘事治療、基模治療、精神分析、心理動力、人本取向、催眠治療、身體經驗創傷療法、產後心理、老年心理、LGBTQ、多元文化、慢性疼痛、睡眠醫學、哀傷輔導、壓力調適、醫療諮商、生涯發展。
...
<title>Counseling Brain &middot; \u81e8\u5e8a\u5fc3\u7406\u8207\u5fc3\u7406\u6cbb\u7642\u6587\u737b\u65e5\u5831 &middot; {date_display}</title>
<meta name="description" content="{date_display} \u81e8\u5e8a\u5fc3\u7406\u8207\u5fc3\u7406\u6cbb\u7642\u6587\u737b\u65e5\u5831\uff0c\u7531 AI \u81ea\u52d5\u5f59\u6574 PubMed \u6700\u65b0\u8ad6\u6587"/>
<style>
  :root {{ --bg: #eef3f8; --surface: #f8faff; --line: #c8d6e5; --text: #1a2a3a; --muted: #5a7089; --accent: #2b6cb0; --accent-soft: #d0e2f2; --card-bg: color-mix(in srgb, var(--surface) 92%, white); }}
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: radial-gradient(circle at top, #f0f5ff 0, var(--bg) 55%, #dce6f0 100%); color: var(--text); font-family: "Noto Sans TC", "PingFang TC", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; overflow-x: hidden; }}
  ...
  .logo {{ width: 48px; height: 48px; border-radius: 14px; background: var(--accent); display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; box-shadow: 0 4px 20px rgba(43,108,176,0.25); }}
  ...
  .badge-count {{ background: rgba(43,108,176,0.06); border: 1px solid var(--line); color: var(--muted); }}
  ...
  .rank-badge {{ background: var(--accent); color: #f0f5ff; font-weight: 700; font-size: 12px; padding: 2px 8px; border-radius: 6px; }}
  ...
  .utility-high {{ color: #2d7a4f; font-size: 11px; font-weight: 600; padding: 2px 8px; background: rgba(45,122,79,0.1); border-radius: 4px; }}
  .utility-mid {{ color: #9f7a2e; font-size: 11px; font-weight: 600; padding: 2px 8px; background: rgba(159,122,46,0.1); border-radius: 4px; }}
  .utility-low {{ color: var(--muted); font-size: 11px; font-weight: 600; padding: 2px 8px; background: rgba(90,112,137,0.08); border-radius: 4px; }}
  .utility-sm {{ font-size: 10px; }}
  .pico-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 12px; padding: 12px; background: rgba(248,250,255,0.8); border-radius: 14px; border: 1px solid var(--line); }}
  .pico-item {{ display: flex; gap: 8px; align-items: baseline; }}
  .pico-label {{ font-size: 10px; font-weight: 700; color: #f0f5ff; background: var(--accent); padding: 2px 6px; border-radius: 4px; flex-shrink: 0; }}
  ...
  .keyword:hover {{ background: rgba(43,108,176,0.18); }}
  ...
  .topic-bar {{ height: 100%; background: linear-gradient(90deg, var(--accent), #5a9fd4); border-radius: 4px; transition: width 0.6s ease; }}
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Counseling Brain &middot; \u81e8\u5e8a\u5fc3\u7406\u8207\u5fc3\u7406\u6cbb\u7642\u6587\u737b\u65e5\u5831</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/counseling-brain">GitHub</a></span>
  </footer>
</div>
</body>
</html>"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate counseling & psychotherapy daily report HTML"
    )
    ...
    if not papers_data or not papers_data.get("papers"):
        ...
        analysis = {
            ...
            "market_summary": "\u4eca\u65e5 PubMed \u66ab\u7121\u65b0\u7684\u81e8\u5e8a\u5fc3\u7406/\u5fc3\u7406\u6cbb\u7642\u6587\u737b\u66f4\u65b0\u3002\u8acb\u660e\u5929\u518d\u67e5\u770b\u3002",
            "top_picks": [],
            "all_papers": [],
            "keywords": [],
            "topic_distribution": {},
